...
def main() -> int:
    ...
    pairs_filter = [p.strip() for p in args.pairs.split(",") if p.strip()] if args.pairs else None
    ...
    with file_lock_or_none("backup") as flock:
        ...
        logger.info("Backup-Job gestartet (ID=%s, via CLI, dry_run=%s, pairs=%s)", job_id, args.dry_run, pairs_filter)

        try:
            summary = run_job(dry_run=args.dry_run, pairs_filter=pairs_filter)
            status = "ok" if summary.get("ok") else "error"
            db.job_finish(job_id, status, summary)
            logger.info("Fertig: %s", json.dumps(summary, ensure_ascii=False, default=str)[:1000])
