"""Minimale SQLite-DB für Job-Tracking. Job = ein backup-Lauf."""
...
_DB_PATH = Path("/opt/rclone-sync/data/rclone-sync.db")
...
class Database:
    def __init__(self, path: Path = _DB_PATH):
        ...
        c0 = sqlite3.connect(str(path), timeout=10, check_same_thread=False)
        try:
    ...
    def job_start(self, kind: str) -> int:
        with self.conn() as c:
            cur = c.execute(
