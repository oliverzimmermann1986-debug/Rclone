...
@pytest.mark.parametrize("flag", ("-ni", "-vn"))
def test_combined_protected_short_flags_are_blocked(flag):
    with pytest.raises(UnsafeRcloneArgument):
        validate_rclone_args([flag])
...
@pytest.mark.parametrize("flag", ("-pass=canary", "-password", "--password"))
@pytest.mark.parametrize("allow_unsafe", (False, True))
def test_short_and_long_credential_flags_are_always_blocked(flag, allow_unsafe):
    with pytest.raises(UnsafeRcloneArgument, match="Zugangsdaten-Flags"):
        validate_rclone_args([flag], allow_unsafe=allow_unsafe)
...
def test_command_text_redacts_flag_and_url_credentials():
    command = (
        ...
        "--header=Authorization:canary-three -pass canary-five "
        "https://user:canary-four@example.test/a"
    )
