...
- **Schutzassistent:** Profil und vorhandenen Push-Datenweg wählen, konkrete Änderungen bestätigen und erste Restore-Stichprobe starten. Gemeinsame bestehende Jobs bleiben unverändert.
...
Das Passwort wird nur für den Login übertragen und nicht gespeichert. Bei WebAuthn verbleibt der private Schlüssel im iCloud-Schlüsselbund, System-Authenticator oder physischen FIDO2-Schlüssel; die App erhält nur einen einmal verwendbaren Sitzungscode. Serveradresse und Benutzername liegen in `UserDefaults`. Mit „Angemeldet bleiben“ werden Sitzung und CSRF-Cookie im nicht synchronisierenden Geräteschlüsselbund gespeichert, getrennt nach Schema, Host, Port und Basispfad. Bestätigtes HTTP darf seine noch gültige Sitzung nach App-Neustart wiederherstellen; HTTPS bleibt empfohlen. Ohne diese Option, nach Abmeldung, Widerruf oder Ablauf ist eine neue Anmeldung nötig. Passkeys benötigen weiterhin eine geeignete HTTPS-Origin. Der Privacy-Manifest deklariert den app-internen `UserDefaults`-Zugriff mit Apples Grund `CA92.1`; Tracking und Datensammlung sind deaktiviert.
